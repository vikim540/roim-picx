<template>
    <div class="relative group">
        <div class="border-2 border-dashed rounded-3xl p-8 sm:p-12 text-center transition-all duration-300 cursor-pointer min-h-[340px] flex flex-col items-center justify-center bg-white dark:bg-gray-800"
            ...
            <input type="file" ref="input" multiple accept="image/*" class="hidden" @change="onInputChange" @click.stop />
            ...
            <template v-if="convertedImages.length === 0">
                ...
                <p class="text-gray-500 dark:text-gray-400 max-w-xs mx-auto text-sm leading-relaxed mb-6">
                    {{ $t('upload.dropzoneHint') }}
                </p>
                <button type="button" @click.stop="triggerFileInput"
                    class="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm shadow-md hover:shadow-lg transition-all flex items-center gap-2">
                    <font-awesome-icon :icon="faPlus" />
                    <span>{{ $t('upload.selectFiles') || '选择图片' }}</span>
                </button>
            </template>
            <template v-else>
                <div class="w-full">
                    <div class="flex justify-between items-center mb-6 px-2" @click.stop>
                        <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                            已选择 {{ convertedImages.length }} 张图片
                        </span>
                        <button type="button" @click.stop="triggerFileInput"
                            class="px-4 py-1.5 rounded-xl bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 font-medium text-xs transition-colors flex items-center gap-1.5 border border-indigo-200 dark:border-indigo-800">
                            <font-awesome-icon :icon="faPlus" />
                            <span>继续添加</span>
                        </button>
                    </div>
